...
timekill.pl $TIMEOUT $MAINCLASS java -ea -Xms2048m\ -Xmx2048m\ -Xss128m \
  $MAINCLASS -nobuiltins -include $CPPDATA/builtins.h $extraincludes \
  -silent -preprocess -cfg newdirectives -include $CPPDATA/nonbooleans.h \
  -cfg macros -cfg includes \
  $@ > $preprocC
